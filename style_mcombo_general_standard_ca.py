...
class MComboGeneralStandardStyle(BoxMarkStyle):
    ...
    def get_style_name(self):
        return "mcombo_general_standard_ca"

    def get_style_description(self):
        return "加拿大MCombo 多箱通用箱唛第一箱 箱唛"
    ...
    def _load_resources(self):
        ...
        self.resources = {
            ...
            'icon_flap_over32kg':   res_base / '重量超32KG提示.png',
            'icon_company_bg_left': general_functions.make_it_pure_black(
                Image.open(res_base / '正唛-公司名称.png').convert('RGBA')),
            'icon_company_bg_right': general_functions.make_it_pure_black(
                Image.open(res_base / '正唛-邮箱.png').convert('RGBA')),
    ...
    def _draw_flap_left(self, pdf: FPDF, sku_config,
        ...
        target_h_mm = 100.0
        icon_max_w_mm = w_mm * 0.8
        icon_w_natural = target_h_mm * icon_width / icon_height
        if icon_w_natural > icon_max_w_mm:
            icon_w_mm = icon_max_w_mm
            icon_h_mm = icon_w_mm * icon_height / icon_width
        else:
            icon_w_mm = icon_w_natural
            icon_h_mm = target_h_mm

        # SKU 文字位于图标下方，宽度不超过面板宽度的 74%
        sku_text = sku_config.sku_name
        max_w_mm = w_mm * 0.74
        sku_size_pt, pil_sku = self._get_font_size(
            sku_text, 'AvantGardeLT-Demi', max_w_mm, sku_config.ppi
        )
        _, sku_top, _, sku_bottom = self._pil_bbox_mm(
            pil_sku, sku_text, sku_config.ppi
        )
        sku_h_mm = sku_bottom - sku_top

        # 将图标 + 间距 + SKU 作为一个整体，在翻盖区域内水平、垂直居中
        content_gap_mm = 10.0
        content_h_mm = icon_h_mm + content_gap_mm + sku_h_mm
        img_x = x_mm + (w_mm - icon_w_mm) / 2.0
        img_y = y_mm + (h_mm - content_h_mm) / 2.0
        sku_center_x = x_mm + w_mm / 2.0
        sku_top_y = img_y + icon_h_mm + content_gap_mm

        def draw_content():
            pdf.image(icon, x=img_x, y=img_y, w=icon_w_mm, h=icon_h_mm)
            self._draw_text_top_center(
                pdf, sku_center_x, sku_top_y, sku_text,
                'AvantGardeLT-Demi', '', sku_size_pt, pil_sku, sku_config.ppi
            )

        if rotate_180:
            panel_center_x = x_mm + w_mm / 2.0
            panel_center_y = y_mm + h_mm / 2.0
            with pdf.rotation(180, panel_center_x, panel_center_y):
                draw_content()
        else:
            draw_content()
    ...
    def _draw_flap_right(self, pdf: FPDF, sku_config,
        ...
        icon_over32kg_path = self.resources['icon_flap_over32kg']  # 替换为重量超32KG提示图标

        # 根据毛重条件动态决定宽度和包含的元素
        if sku_config.side_text['gw_value'] * 2.20462 > 32:  # 超过 32kg，显示两个图标
            # 两个图标并排显示
            icon_keepboxes_width = w_mm * 0.43
            icon_over32kg_width = w_mm * 0.27
            elements = [
                engine.Image(icon_keepboxes_path, width=icon_keepboxes_width),
                engine.Image(icon_over32kg_path, width=icon_over32kg_width)
            ]
        else:
    ...
    def _draw_front_panel(self, pdf: FPDF, sku_config, x_mm, y_mm, w_mm, h_mm):
        ...
        icon_company_w_mm = (
            icon_company_h_mm * icon_company.width / icon_company.height
        )
    ...
    def _draw_side_text_content(self, pdf: FPDF, sku_config,
        ...
        label_px   = int(h_mm * px_per_mm * 0.078)
