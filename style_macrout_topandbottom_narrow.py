...
class MacroutTopAndBottomNarrowStyle(BoxMarkStyle):
    """Macrout 窄箱天地盖样式 (fpdf2 版)"""

    def get_style_name(self):
        return "macrout_topandbottom_narrow"

    def get_style_description(self):
        return "Macrout 窄箱天地盖箱唛样式"
    ...
    def _draw_top_panel(self, pdf: FPDF, sku_config, x_mm, y_mm, w_mm, h_mm):
        ...
        product_pt, _ = self._get_font_size(
            product_text, 'ITC Avant Garde Gothic Demi Cyrillic', w_mm * 0.40, ppi)
        sku_pt, _ = self._get_font_size(
            sku_text, 'Calibri Bold', w_mm * 0.43, ppi)
        color_pt, _ = self._get_font_size(
            color_text, 'Arial Regular', w_mm * 0.10, ppi)
        ...
        bottom_row = engine.Row(
            ...
            children=[
                ...
                engine.Image(self.resources['icon_attention'], width=w_mm * 0.15),
            ]
        )
    ...
    def _draw_label(self, pdf: FPDF, sku_config,
        ...
        # 产地文字大小
        origin_text_size_px = int(h_mm * 0.28 * px_per_mm)
