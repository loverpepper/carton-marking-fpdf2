...
from layout_engine import Image as LEImage, Text as LEText, Column


@StyleRegistry.register
class MComboVerticalStyle(BoxMarkStyle):
    """MCombo 通用箱唛第二箱三箱箱唛样式 (fpdf2 版)"""

    def get_style_name(self):
        return "mcombo_general_vertical_ca"

    def get_style_description(self):
        return "加拿大MCombo 多箱通用箱唛第二箱三箱 箱唛"
    ...
    def get_layout_config_mm(self, sku_config):
        """MCombo 通用箱唛第二三箱样式 - 12块布局（4列3行），单位 mm。
    ...
    def _load_resources(self):
        """加载 MCombo 通用箱唛第二三箱样式的图片资源（与标准样式相同）"""
    ...
    def _draw_flap(self, pdf: FPDF, sku_config,
        ...
        sku_max_w_mm = w_mm * 0.74
        sku_size_pt, _ = self._get_font_size(
            sku_text, 'AvantGardeLT-Demi', sku_max_w_mm, sku_config.ppi
        )
        sku_element = LEText(
            sku_text,
            font_family='AvantGardeLT-Demi',
            font_size_pt=sku_size_pt,
            font_path=self.font_paths['AvantGardeLT-Demi'],
            ppi=sku_config.ppi,
        )

        col = Column(
            [icon_upper, icon_lower, sku_element],
            spacing=20.0,
            align='center',
        )
