...
class MComboStandardStyle(BoxMarkStyle):
    ...
    def get_style_name(self):
        return "new_market_standard_hongqi"

    def get_style_description(self):
        return "new_market 鸿启专属箱唛样式 第二三箱"
    ...
    def draw_to_pdf(self, pdf: FPDF, sku_config):
        ...
        self._draw_side_panel_rotated_ccw(pdf, sku_config, x, y, w, h,
                                            show_legal=True)
        ...
        self._draw_flap_left(pdf, sku_config, *layout['flap_top_front1'], rotate_180=False)
        self._draw_flap_left(pdf, sku_config, *layout['flap_btm_front1'], rotate_180=True)

        # Right flaps
        self._draw_flap_right(pdf, sku_config, *layout['flap_top_front2'], rotate_180=True)
        self._draw_flap_right(pdf, sku_config, *layout['flap_btm_front2'], rotate_180=False)

        # Side up/down — blank (already filled with background)

    def _draw_side_panel_rotated_ccw(self, pdf, sku_config,
                                     x_mm, y_mm, w_mm, h_mm, show_legal):
        """Draw a side panel rotated 90 degrees counter-clockwise in place."""
        center_x = x_mm + w_mm / 2.0
        center_y = y_mm + h_mm / 2.0

        # Swap the drawing frame dimensions so the rotated artwork still fits
        # exactly within the side panel's original bounds.
        rotated_x = center_x - h_mm / 2.0
        rotated_y = center_y - w_mm / 2.0
        with pdf.rotation(90, x=center_x, y=center_y):
            self._draw_side_panel(
                pdf, sku_config,
                rotated_x, rotated_y, h_mm, w_mm,
    ...
    def _load_resources(self):
        ...
        self.resources = {
            ...
            # 正唛两种logo
            'icon_front_logo_elegue': Image.open(self.res_base / '正唛logo-ELEGUE.png').convert('RGBA'),
            'icon_front_logo_mcombo': Image.open(self.res_base / '正唛logo-MCombo.png').convert('RGBA'),
    ...
    def _draw_side_panel(self, pdf, sku_config,
                              o_x, o_y, eff_w, eff_h,
                              # h_left_mm 是侧唛底部黑框高度
                              # logo_h_mm 是侧唛右上logo高度
                              show_legal, h_left_mm = 100.0, logo_h_mm = 50.0):
        ...
        current_x = o_x + 20.0  # 1 cm from left
        ...
        if show_legal and getattr(sku_config, 'legal_data', None):
            ...
            legal_bottom_limit_y = panel_bottom - h_left_mm - gap_above_bar_mm
