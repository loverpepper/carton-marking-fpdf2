...
class MComboStandardStyle(BoxMarkStyle):
    """澳洲 MCombo 标准箱唛样式 (fpdf2 版)"""

    def get_style_name(self):
        return "AU-mcombo_standard"
    ...
    def _load_resources(self):
        ...
        self.resources = {
            ...
            'icon_overweight':       res_base / '重量超35KG提示.png',
    ...
    def draw_to_pdf(self, pdf: FPDF, sku_config):
        ...
        self._draw_flap(pdf, sku_config, x0, 0.0,  l_mm, half_w_mm, rotate_180=False)
        self._draw_flap(pdf, sku_config, x2, y2,  l_mm, half_w_mm, rotate_180=True)
    ...
    def _draw_flap(self, pdf: FPDF, sku_config,
        ...
        icon = self.resources['icon_overweight']

        main_col = engine.Column(
            children = [
                engine.Image(icon, width= w_mm * 0.57)
                ],
            fixed_height= h_mm,
            fixed_width= w_mm,
            align='center',
            justify='center',
            spacing = 20, # 图片和SKU之间间距 20mm
        )
        
        main_col.layout(x_mm, y_mm)
        
        if rotate_180:
            with pdf.rotation(180, x_mm + w_mm/2, y_mm + h_mm/2):
                main_col.render(pdf=pdf)
        else:
            main_col.render(pdf=pdf)
    ...
    def _draw_side_text_content(self, pdf: FPDF, sku_config,
        ...
        print(f"生成的侧唛SKU条形码宽度是 {sku_bc_w_mm:.2f} mm")
